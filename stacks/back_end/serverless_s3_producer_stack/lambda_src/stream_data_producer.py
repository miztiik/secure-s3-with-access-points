...
def put_object(_pre, data):
    try:
        _r = _s3.put_object(
            ...
            Key=f"{_pre}/dt={datetime.datetime.now().strftime('%Y_%m_%d')}/{datetime.datetime.now().strftime('%s%f')}.json",
...
def lambda_handler(event, context):
    ...
    _usr_names = ["Aarakocra", "Aasimar", "Githzerai", "Gnoll", "Gnome",
                  "Goblin", "Goliath", "Hag", "Half-Elf", "Half-Orc", "Halfling"]
    ...
    try:
        ...
        inventory_evnts = 0
        t_sales = 0
        while context.get_remaining_time_in_millis() > 500:
