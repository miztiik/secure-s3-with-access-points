...
class S3ConsumerOnEC2Stack(cdk.Stack):
    ...
    def __init__(
        ...
        **kwargs
        ...
        try:
            with open("stacks/back_end/s3_consumer_on_ec2_stack/bootstrap_scripts/deploy_app.sh",
                      encoding="utf-8",
                      mode="r"
                      ) as f:
                user_data = f.read()
        ...
        output_3 = cdk.CfnOutput(
            ...
            "Ec2ConsumerRoleArn",
            value=(
                f"{self._instance_role.role_arn}"
            ),
            description=f"Ec2 Consumer Role Arn"
        )
        output_4 = cdk.CfnOutput(
            self,
            "SampleS3CopyCommand",
            value=(
                f"aws s3api put-object --key AP_PREFIX/KEY_NAME --bucket EC2_CONSUMER_ACCESS_POINT_ARN --body FILE_PATH"
            ),
            description=f"aws s3api put-object --key sales_event/msg.json --bucket arn:aws:s3:us-east-1:111122223333:accesspoint/ec2-consumer --body messages"
        )
